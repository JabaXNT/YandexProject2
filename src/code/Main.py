...
class ChangeCard(QDialog, Ui_Dialog_2):  # Изменение уже существующей карты
    ...
    def save(self):
        ...
        self.main.day_new = ''.join(self.bday_new.text().strip())
        ...
        if self.main.name_new == '':
            ...
            return
        if self.main.day_new == '':
            self.label.setText(lang.bday_input)
            return
        variable2 = [self.main.name_new, self.main.sur_new, self.main.pat_new, self.main.day_new,
                     self.main.id_numb[0]]
        try:
            self.res.execute(
                "UPDATE Patients SET name = ?, surname = ?, patronymic = ?, birthday = ? WHERE id = ?",
                variable2)
...
class AddCard(QDialog, Ui_Dialog_1):  # Создание новой карты пациента
    ...
    def close_win(self):
        ...
        self.main.day_text = ''.join(self.bday.text().strip())
        ...
        if self.main.name_text == '':
            ...
            return
        if self.main.day_text == '':
            self.msg.setText(lang.bday_input)
            return
        variable = (
            self.main.id_text, self.main.sur_text, self.main.name_text, self.main.pat_text, self.main.day_text)
        try:
